...
public class ContractTrackerRenderer {
    ...
    // 平滑参数
    private static final float SMOOTHING_FACTOR = 0.2f; // 平滑系数，值越小越平滑
    ...
    private static void onRenderWorldLast(RenderWorldLastEvent event) {
        ...
        try {
            ...
            for (Contract contract : contractManager.getAllContracts()) {
                ...
                // 计算距离
                double distance = smoothedPlayerPos.distanceTo(targetVec);
                ...
                if (distance <= MAX_LINE_LENGTH) {
                    ...
                    // 计算方向向量
                    Vec3 direction = targetVec.subtract(smoothedPlayerPos).normalize();
                    // 计算终点位置（100米处）
